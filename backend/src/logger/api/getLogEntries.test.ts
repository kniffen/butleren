...
describe('getLogEntries()', () => {
  ...
  test('It should get log entries for a specific date', async () => {
    ...
    await getLogEntries(request, response);
  ...
  test('It should return 400 if date query parameters is missing', async () => {
    ...
    await getLogEntries(req, response);
  ...
  test('It should return 500 if an error occurs while reading or parsing the log file', async () => {
    existsSyncSpy.mockReturnValueOnce(true);
    ...
    expect(response.send).toHaveBeenCalledWith('File read error');
  });
});
