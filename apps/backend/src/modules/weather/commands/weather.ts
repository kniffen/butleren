...
import { getOpenWeatherData } from '../utils/getOpenWeatherData';
import { getGuildSettings } from '../../../discord/database/getGuildSettings';
import { createWeatherEmbed } from '../utils/createWeatherEmbed';

const slashCommandBuilder =
  new SlashCommandBuilder()
    .setName('weather')
    .setDescription('Get a weather report')
...
const execute = async (commandInteraction: ChatInputCommandInteraction): Promise<void> => {
  try {
    ...
    const openWeatherData = await getOpenWeatherData(location.lat, location.lon);
    const embed = createWeatherEmbed(guildSettings, openWeatherData, location);

    await commandInteraction.editReply({ embeds: [embed] });

  } catch (error: unknown) {
    logError('Weather', 'Unable to get weather data', { error });
