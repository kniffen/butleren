...
async function testLocation(location: string) {
...
export default async function setLocation(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const location = interaction.options.get('location')?.value?.toString();

    if (!location || !(await testLocation(location))) {
